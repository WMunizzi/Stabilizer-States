...
A Mathematica package for stabilizer states, stabilizer circuits, and reachability diagrams, by William Munizzi

April 2022 version 1 (Logged Changes at End of Package)

Some General Notes:

     States in this program are represented using the bit-address formalism. In this way, an N-dimensional qubit |\[CapitalPsi]\[RightAngleBracket] is
represented by an ordered string of 2^n components, selected from 0,1,-1,I,-I. The n^th component of this string corresponds
to the coefficient multiplying the n^th ket in the ket-expansion of |\[CapitalPsi]\[RightAngleBracket], e.g. |\[Psi]\[RightAngleBracket] = (1,0,0,-I) = 1|00\[RightAngleBracket]+0|01\[RightAngleBracket]+0|10\[RightAngleBracket]-I|11\[RightAngleBracket].

Note: The ordering of qubits within a ket is given from RIGHT TO LEFT, e.g. the rightmost digit corresponds to the first
qubit of the system, while the leftmost digit represents the n^th qubit of an n-qubit syste. The binary string within each
ket directly equates the left-acting tensor product of single-qubit states.


File repository with stabilizer states, map of all states, and reachability graphs up to five qubits is available at:
https://github.com/WMunizzi/StabilizerStateData

Files may be opened or loaded directly into Mathematica notebook.
...
NumberSelectedStateGeneratorWrite::usage= "StabilizerStateSetGenerator[dim] returns the full dim-qubit set of stabilizer states. Set is returned as a list of vectors normalized to the first component.";
Begin["`Private`"];
...
ReachabilityGraphBuilder::usage= "ReachabilityGraphBuilder[SetOfStatesWithGates_,GatesToBeDisplayed_,EntropyVectorList_,EdgeLabelBoolean_,VertexLabelChoice_,VertexCoords_] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
Package launched on April 14, 2022.
