...
Begin["`Private`"];
SvonNeumannBinary[StateKet_,SubsystemList_]:= Block[{NonzeroEigenlist,NormKet = StateKet/Sqrt[Total[Abs[StateKet]]][[1]]},
...
Begin["`Private`"];
ReducedEntropyVectorBuilder[StateKet_]:=Block[{dims = Log[2,Length[StateKet]]},
...
CliffordGenerators::usage= "CliffordGenerators[n] returns the set of generators for the n-qubit Clifford group.";
Begin["`Private`"];
...
Begin["`Private`"];
StabilizerStateSetGenerator[Dimension_]:=Block[{PredictedCount = ((2^Dimension)*Product[(2^num)+1,{num,1,Dimension}]),UpdatedStabilizerStateSet={ReplacePart[ConstantArray[{0},2^Dimension],1->{1}]},CheckSet={ReplacePart[ConstantArray[{0},2^Dimension],1->{1}]},NormSet={} ,TotalGateSet=CliffordGenerators[Dimension]},

While[Length[UpdatedStabilizerStateSet]<PredictedCount,

NormSet = Flatten[Table[NormKet[TotalGateSet[[k]].CheckSet[[l]]],{k,Length[TotalGateSet]},{l,Length[CheckSet]}],1];
...
InitialStateGraphFinder::usage= "InitialStateGraphFinder[InitialState_,GateSet_,DisplayGraphBoolean_]";
Begin["`Private`"];
InitialStateGraphFinder[InitialState_,GateSet_,DisplayGraphBoolean_]:=
Block[{UpdatedStabilizerStateSet={InitialState},CheckSet={InitialState},NormSet={},TotalGateSet={},GateSetLength,TestBoolean=False,j,kk,ll},

For[kk=1,kk<Length[GateSet]+1,kk++,If[GateSet[[kk,1]]=="H",AppendTo[TotalGateSet,NonNormHadamardAction[(Log[2,Length[InitialState]]),GateSet[[kk,2,1]]]],If[GateSet[[kk,1]]=="P",AppendTo[TotalGateSet,PhaseAction[(Log[2,Length[InitialState]]),GateSet[[kk,2,1]]]],AppendTo[TotalGateSet,CNOTAction[(Log[2,Length[InitialState]]),GateSet[[kk,2]]]],
Print["Unable to interpret gate."]]]];

GateSetLength = Length[TotalGateSet];

Print["Gates Compiled."];

While[TestBoolean== False,

UpdatedStabilizerStateSet = Union[Join[UpdatedStabilizerStateSet,CheckSet]];

CheckSet = Flatten[Table[TotalGateSet[[kk]].CheckSet[[ll]],{kk,GateSetLength},{ll,Length[CheckSet]}],1];

NormSet={};
Table[AppendTo[NormSet,((CheckSet[[i]][[FirstPosition[Flatten[Transpose[CheckSet[[i]]]],_?(##!= 0&)][[1]]]][[1]])^(-1))*CheckSet[[i]]] ,{i,Length[CheckSet]}];

CheckSet = Union[NormSet];

Print[Length[UpdatedStabilizerStateSet]];
TestBoolean = SubsetQ[UpdatedStabilizerStateSet,CheckSet];
];
Print[Length[UpdatedStabilizerStateSet]];
UpdatedStabilizerStateSet];
End[];
