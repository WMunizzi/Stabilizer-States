...
SvonNeumannBinary::usage="SvonNeumannBinary[StateKet,{Subsystem1,Subsystem2,...}] computes the entanglement entropy of the requested subsystem(s) of StateKet.";
Begin["`Private`"];
...
(* ::Section::Closed:: *)
(*Quantum Logic Gates (Documentation Still Needed)*)


HadamardGate::usage= "HadamardGate[StateKet, n] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
NonNormHadamardGate::usage= "NonNormHadamardGate[StateKet, n] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
NonNormHadamardGate[StateKet_,AffectedQubit_]:=Module[{},NonNormHadamardAction[Log[2,Length[StateKet]],AffectedQubit].StateKet];
End[];

NormalizedHadamardGate::usage= "NormalizedHadamardGate[StateKet, n] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
NonNormHadamardAction::usage= "NonNormHadamardAction[Dimension, n] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
UnequalHadamardGate::usage= "UnequalHadamardGate[StateKet, n, P] returns a ket vector representing the system after a Hadamard gate of nonuniform diagonal is applied to the nth qubit. The specific superposition split is given by p, which is the probability of the system being in the spin-up state.";
Begin["`Private`"];
...
CNOTGate::usage= "To be explained.";
Begin["`Private`"];
...
CNOTAction::usage= "CNOTAction[Dimension, {control qubit, target qubit}] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
PhaseGate::usage= "To be explained.";
Begin["`Private`"];
PhaseGate[StateKet_,AffectedQubit_]:=Module[{},PhaseAction[Log[2,Length[StateKet]],AffectedQubit].StateKet];
End[];

PhaseAction::usage= "PhaseAction[Dimension, n] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
ControlledPhaseGate::usage= "To be explained.";
Begin["`Private`"];
...
(* ::Section:: *)
(*Quantum Circuits (Documentation Needed)*)


StabilizerCircuit::usage= "StabilizerCircuit[StateKet, OperationsList, vQubitList] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
StabilizerStateSetGenerator::usage= "StabilizerStateSetGenerator[StateKet, n] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
For[ii=1,ii<Length[SingleQubits]+1,ii++,AppendTo[TotalGateSet,NonNormHadamardAction[Dimension,SingleQubits[[ii,1]]]];AppendTo[TotalGateSet,PhaseAction[Dimension,SingleQubits[[ii,1]]]]];
...
SelectiveStabilizerStateSetGeneratorWrite::usage= "SelectiveStabilizerStateSetGeneratorWrite[StateKet, n] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
SelectiveStabilizerStateSetGeneratorWrite[StartingAddress_,NumberOfDesiredStates_,FileDestination_]:=Block[{UpdatedStabilizerStateSet={StartingAddress},CheckSet={StartingAddress},NormSet={},TestState ,SingleQubits = Permutations[Range[Log[2,Length[StartingAddress]]],{1}],PairedQubits = Permutations[Range[Log[2,Length[StartingAddress]]],{2}],TotalGateSet={},ii,jj,kk,ll},

For[ii=1,ii<Length[SingleQubits]+1,ii++,AppendTo[TotalGateSet,NonNormHadamardAction[Log[2,Length[StartingAddress]],SingleQubits[[ii,1]]]];AppendTo[TotalGateSet,PhaseAction[Log[2,Length[StartingAddress]],SingleQubits[[ii,1]]]]];
...
SelectiveStabilizerStateSetGenerator::usage= "SelectiveStabilizerStateSetGenerator[StateKet, n] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
SelectiveStabilizerStateSetGenerator[StartingAddress_,NumberOfDesiredStates_]:=Block[{UpdatedStabilizerStateSet={StartingAddress},CheckSet={StartingAddress},NormSet={},TestState ,SingleQubits = Permutations[Range[Log[2,Length[StartingAddress]]],{1}],PairedQubits = Permutations[Range[Log[2,Length[StartingAddress]]],{2}],TotalGateSet={},ii,jj,kk,ll},
...
GateSelectedStateGenerator::usage= "GateSelectedStateGenerator[StateKet, n] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
For[kk=1,kk<Length[DesiredGateSet]+1,kk++,If[DesiredGateSet[[kk,1]]=="H",AppendTo[TotalGateSet,NonNormHadamardAction[(Log[2,Length[StartingAddress]]),DesiredGateSet[[kk,2,1]]]],If[DesiredGateSet[[kk,1]]=="P",AppendTo[TotalGateSet,PhaseAction[(Log[2,Length[StartingAddress]]),DesiredGateSet[[kk,2,1]]]],AppendTo[TotalGateSet,CNOTAction[(Log[2,Length[StartingAddress]]),DesiredGateSet[[kk,2]]]],Print["Unable to interpret gate."]]]];
...
StabilizerStateGeneratorWithEntropy::usage= "StabilizerStateGeneratorWithEntropy[StateKet, n] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
For[ii=1,ii<Length[SingleQubits]+1,ii++,AppendTo[TotalGateSet,NonNormHadamardAction[dim,SingleQubits[[ii,1]]]]];
...
GroupBuilder::usage="GroupBuilder[List,FirstIndex,SecondIndex] gives back List with elements at FirstIndex and SecondIndex swapped.";
Begin["`Private`"];
...
StabilizingSubgroupFinder::usage="StabilizingSubgroupFinder[List,FirstIndex,SecondIndex] gives back List with elements at FirstIndex and SecondIndex swapped.";
Begin["`Private`"];
...
LoopCountWords::usage="LoopCountWords[List,FirstIndex,SecondIndex] gives back List with elements at FirstIndex and SecondIndex swapped.";
Begin["`Private`"];
...
LoopCountLoops::usage="LoopCountLoops[List,FirstIndex,SecondIndex] gives back List with elements at FirstIndex and SecondIndex swapped.";
Begin["`Private`"];
...
SimpleReachabilityGraphBuilder::usage= "SimpleReachabilityGraphBuilder[StateKet, OperationsList,QubitList] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
ReachabilityGraphBuilder::usage= "ReachabilityGraphBuilder[StateKet, OperationsList,QubitList] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
OperationLocator::usage= "OperationLocator[StateKet, OperationsList,QubitList] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
GraphFinderExactVertex::usage= "GraphFinderExactVertex[StateKet, OperationsList,QubitList] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
If[DesiredGates[[jjj,1]]=="H",AppendTo[DesiredGateSet,NonNormHadamardAction[(Log[2,Length[InitialState]]),DesiredGates[[jjj,2,1]]]],If[DesiredGates[[jjj,1]]=="P",AppendTo[DesiredGateSet,PhaseAction[(Log[2,Length[InitialState]]),DesiredGates[[jjj,2,1]]]],AppendTo[DesiredGateSet,CNOTAction[(Log[2,Length[InitialState]]),DesiredGates[[jjj,2]]]],Print["Unable to interpret gate."]]]];

StateSet = SelectiveStabilizerStateSetGenerator[InitialState,DesiredStatesToSearch];
...
GraphFinder::usage= "GraphFinder[StateKet, OperationsList,QubitList] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
InteractiveReachabilityGraphBuilder::usage= "InteractiveReachabilityGraphBuilder[StateKet, OperationsList, vQubitList] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
CycleFinder::usage= "CycleFinder[StateKet, OperationsList, vQubitList] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
ConnectedSubgraphGenerator::usage= "ConnectedSubgraphGenerator[StateKet, OperationsList, vQubitList] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
SequenceGenerator::usage= "SequenceGenerator[StateKet, OperationsList, vQubitList] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
SubAdditivityCheck::usage= "SubAdditivityCheck[StateKet, OperationsList, vQubitList] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
MutualInformationMeasure::usage= "MutualInformationMeasure[StateKet, OperationsList, vQubitList] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
MMICheckerStateInput::usage= "MMICheckerStateInput[StateKet, OperationsList, vQubitList] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
MMICheckerEntropyInput::usage= "MMICheckerEntropyInput[StateKet, OperationsList, vQubitList] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
HolographicChecker::usage= "HolographicChecker[StateKet, OperationsList, vQubitList] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
...
HolographicStateFinder::usage= "HolographicStateFinder[StateKet, OperationsList, vQubitList] returns a ket vector representing the system after a Hadamard gate is applied to the nth qubit of StateKet.";
Begin["`Private`"];
